...
// ─── Additional MC distribution samplers ────────────────
// All samplers return Float64Array(n) of "unit" random deviates that are
// then scaled by the per-parameter uncertainty `unc` in the form:
//   exponent = nom + unc * r[i]
// They are normalised so this scaling has the conventional meaning of
// "± one unc" matching the spread the user typed in.

// Uniform on [-1, 1]   (so unc acts as a half-width)
function randUniformArray(n) {
  const a = new Float64Array(n);
  for (let i = 0; i < n; i++) a[i] = Math.random() * 2 - 1;
  return a;
}

// Symmetric triangular on [-1, 1] with mode at 0
function randTriangularArray(n) {
  const a = new Float64Array(n);
  for (let i = 0; i < n; i++) {
    const u = Math.random();
    a[i] = u < 0.5 ? -1 + Math.sqrt(2 * u) : 1 - Math.sqrt(2 * (1 - u));
  }
  return a;
}

// Log-normal multiplier deviate: r = exp(N(0, 1)) - E[exp(N(0,1))]
// Centred so mean ≈ 0; positively-skewed, heavy right tail.
function randLognormalArray(n) {
  const a = new Float64Array(n);
  const shift = Math.exp(0.5); // mean of standard log-normal
  for (let i = 0; i < n; i++) {
    a[i] = Math.exp(randn()) - shift;
  }
  return a;
}

// Beta-PERT (symmetric, min=-1, mode=0, max=1, λ=4 → Beta(3,3) on [0,1])
// Generated by rejection sampling against the Beta(3,3) PDF.
function randBetaPERTArray(n) {
  const a = new Float64Array(n);
  // Beta(3,3) pdf: 30 x^2 (1-x)^2, max value = 30*(1/2)^4 = 1.875
  const pdfMax = 1.875;
  for (let i = 0; i < n; i++) {
    let x, u;
    do {
      x = Math.random();
      u = Math.random() * pdfMax;
    } while (u > 30 * x * x * (1 - x) * (1 - x));
    a[i] = 2 * x - 1;
  }
  return a;
}

// Discrete: takes the values {-1, 0, +1} with equal probability (1/3 each)
function randDiscreteArray(n) {
  const a = new Float64Array(n);
  for (let i = 0; i < n; i++) {
    const r = Math.random();
    a[i] = r < 1/3 ? -1 : (r < 2/3 ? 0 : 1);
  }
  return a;
}

// Exponential with rate 1 (mean 1), shifted so mean ≈ 0
function randExponentialArray(n) {
  const a = new Float64Array(n);
  for (let i = 0; i < n; i++) {
    a[i] = -Math.log(1 - Math.random()) - 1;
  }
  return a;
}
...
function sampleDist(dist, nsample) {
  switch (dist) {
    case "flat": {
      // legacy: U(0,1) — kept for backwards compatibility
      const r = new Float64Array(nsample);
      for (let i = 0; i < nsample; i++) r[i] = Math.random();
      return r;
    }
    case "normal truncated": {
      const r = randnArray(nsample);
      for (let i = 0; i < nsample; i++) {
        while (Math.abs(r[i]) > 1.5) r[i] = randn();
      }
      return r;
    }
    case "uniform":     return randUniformArray(nsample);
    case "triangular":  return randTriangularArray(nsample);
    case "lognormal":   return randLognormalArray(nsample);
    case "beta-pert":   return randBetaPERTArray(nsample);
    case "discrete":    return randDiscreteArray(nsample);
    case "exponential": return randExponentialArray(nsample);
    case "normal":
    default:            return randnArray(nsample);
  }
}
...
function monteCarloThreshold(inputs, params, dist, nsample) {
  ...
  for (let ki = 0; ki < nk; ki++) {
    rands.push(sampleDist(dist, nsample));
  }
